...
class ControllerExtensionPaymentAlphacardPGS extends Controller {
 /*   public function index() {
        ...
        $data['onclick'] = $this->config->get('payment_alphacard_pgs_onclick');

        $data['error_session'] = '';
        $data['error_event_missing'] = $this->language->get('error_event_missing');

        if (stripos($this->config->get('payment_alphacard_pgs_merchant'), 'TEST') !== FALSE) {
        ...
        if ($this->customer->isLogged()) {
            $data['cards'] = $this->model_extension_payment_alphacard_pgs->getCards($this->customer->getId());
        } else {
            $data['cards'] = array();
        }
        ...
        if (
            ...
            $this->model_extension_payment_alphacard_pgs->initCheckoutSession($this->session->data['order_id'], $this->session->data['currency'])
        ) {
    ...
    public function init($route) {
        ...
        $this->document->addScript($this->url->link('extension/payment/alphacard_pgs/js', '', true));
    }
    ...
    public function js() {
        ...
        $data['complete'] = $this->url->link('extension/payment/alphacard_pgs/success', '', true);
        $data['cancel'] = $this->url->link('checkout/checkout', '', true);
        
        $this->load->model('extension/payment/alphacard_pgs');

        $data['checkout_script'] = $this->model_extension_payment_alphacard_pgs->getGateway() . '/checkout/version/' . $this->model_extension_payment_alphacard_pgs->getApiVersion() . '/checkout.js';

        $this->response->addHeader("Content-Type:application/javascript");
        $this->response->setOutput($this->load->view('extension/payment/alphacard_pgs_js', $data));
    }
    ...
    public function checkout() {
        ...
        $json = array(
            'redirect' => $this->url->link('extension/payment/alphacard_pgs/success', '', true)
        );
        ...
        if ($this->request->server['REQUEST_METHOD'] != 'POST' || empty($this->request->post['token']) || @base64_decode($this->request->post['token']) === FALSE) {
            ...
            if ($this->config->get('payment_alphacard_pgs_checkout') == 'pay') {
                $configuration['apiOperation'] = 'PAY';
            } else {
                $configuration['apiOperation'] = 'AUTHORIZE';
            }

            $response = $this->model_extension_payment_alphacard_pgs->api('order/' . $this->session->data['order_id'] . '/transaction/1', $configuration, 'PUT');
    ...
    public function success() {
        ...
        if (
            !empty($this->session->data['order_id']) &&
            !empty($this->session->data['alphacard_pgs']['session']['id']) &&
            !empty($this->session->data['alphacard_pgs']['successIndicator']) &&
            !empty($this->request->get['resultIndicator']) &&
            $this->session->data['alphacard_pgs']['successIndicator'] === $this->request->get['resultIndicator']
        ) {
            $session_data = $this->model_extension_payment_alphacard_pgs->api('session/' . $this->session->data['alphacard_pgs']['session']['id']);
            ...
            if (!empty($session_data['billing']['address']) && !empty($session_data['interaction']['displayControl']['billingAddress']) && in_array($session_data['interaction']['displayControl']['billingAddress'], array('MANDATORY', 'OPTIONAL'))) {
                $this->model_extension_payment_alphacard_pgs->editOrderPaymentDetails($this->session->data['order_id'], $session_data['billing']['address']);
            }

            if ($this->config->get('payment_alphacard_pgs_tokenize') && empty($session_data['sourceOfFunds']['token'])) {
                $this->model_extension_payment_alphacard_pgs->saveToken($this->session->data['order_id'], token(40), $this->session->data['alphacard_pgs']['session']['id'], $session_data['sourceOfFunds']);
            }
        }

        $this->model_extension_payment_alphacard_pgs->clearCheckoutSession();
    ...
    public function callback() {
        ...
        if ($this->config->get('payment_alphacard_pgs_debug_log')) {
            $log_data = $this->model_extension_payment_alphacard_pgs->getNotificationLogData();
        ...
        $transaction_info = $this->model_extension_payment_alphacard_pgs->parseWebhookNotification();

        if ($transaction_info) {
            $transaction_info['partnerSolutionId'] = $this->model_extension_payment_alphacard_pgs->getPartnerSolutionId();
            ...
            $is_transaction_logged = $this->model_extension_payment_alphacard_pgs->isTransactionLogged($transaction_info['transaction']['id'], $transaction_info['order']['reference']);

            if (!$is_transaction_logged) {
                $this->model_extension_payment_alphacard_pgs->saveTransaction($transaction_info);

                // last check if everything went okay
                $is_transaction_logged = $this->model_extension_payment_alphacard_pgs->isTransactionLogged($transaction_info['transaction']['id'], $transaction_info['order']['reference']);
            } else {
                $this->model_extension_payment_alphacard_pgs->updateTransaction($transaction_info);
        ...
        if (!$is_transaction_logged) {
            ...
            switch ($risk_gateway_code) {
                case 'REJECTED' : {
                    $this->model_extension_payment_alphacard_pgs->addOrderHistory(array(
                        'order_id' => (int)$transaction_info['order']['reference'],
                        'order_status_id' => (int)$this->config->get('payment_alphacard_pgs_risk_rejected_order_status_id'),
                        'message' => sprintf(
                ...
                case 'REVIEW_REQUIRED' : {
                    ...
                    if (!$decision_exists || $decision_skipped) {
                        ...
                        $this->model_extension_payment_alphacard_pgs->addOrderHistory(array(
                            'order_id' => (int)$transaction_info['order']['reference'],
                            'order_status_id' => (int)$this->config->get('payment_alphacard_pgs_risk_review_' . strtolower($transaction_info['risk']['response']['review']['decision']) . '_order_status_id'),
                            'message' => sprintf(
                                $this->language->get('text_callback'),
                                $transaction_info['transaction']['type'],
        ...
        if ($transaction_info['response']['gatewayCode'] == 'APPROVED') {
            switch ($transaction_info['transaction']['type']) {
                ...
                case 'AUTHORIZATION_UPDATE' : {
                    $order_status_id = $this->config->get('payment_alphacard_pgs_approved_authorization_order_status_id');
                } break;
                case 'CAPTURE' : {
                    $order_status_id = $this->config->get('payment_alphacard_pgs_approved_capture_order_status_id');
                } break;
                case 'PAYMENT' : {
                    $order_status_id = $this->config->get('payment_alphacard_pgs_approved_payment_order_status_id');
                } break;
                case 'REFUND_REQUEST' :
                case 'REFUND' : {
                    $order_status_id = $this->config->get('payment_alphacard_pgs_approved_refund_order_status_id');
                } break;
                case 'VOID_AUTHORIZATION' : 
                case 'VOID_CAPTURE' : 
                case 'VOID_PAYMENT' : 
                case 'VOID_REFUND' : {
                    $order_status_id = $this->config->get('payment_alphacard_pgs_approved_void_order_status_id');
                } break;
                case 'VERIFICATION' : {
                    $order_status_id = $this->config->get('payment_alphacard_pgs_approved_verification_order_status_id');
                } break;
            }
        } else {
            $order_status_id = $this->config->get('payment_alphacard_pgs_' . strtolower($transaction_info['response']['gatewayCode']) . '_order_status_id');
        }
    ...
    protected function validate_callback() {
        ...
        if (!$this->config->get('payment_alphacard_pgs_notification_secret') || $this->config->get('payment_alphacard_pgs_notification_secret') != $this->model_extension_payment_alphacard_pgs->getHeaderVar('HTTP_X_NOTIFICATION_SECRET')) {
    ...
    protected function configuration($hosted = true) {
        ...
        // Limitations taken from here: https://eu-gateway.alphacard.com/api/documentation/apiDocumentation/checkout/version/latest/function/configure.html?locale=en_US
        ...
        if ($hosted && $this->config->get('payment_alphacard_pgs_google_analytics_property_id')) {
            $c['interaction']['googleAnalytics']['propertyId'] = $this->config->get('payment_alphacard_pgs_google_analytics_property_id');
        }
        ...
        if ($hosted) {
            ...
            $c['merchant'] = $this->config->get('payment_alphacard_pgs_merchant');
        ...
        $c['partnerSolutionId'] = $this->model_extension_payment_alphacard_pgs->getPartnerSolutionId();

        $c['session']['id'] = $this->session->data['alphacard_pgs']['session']['id'];
        $c['session']['version'] = $this->session->data['alphacard_pgs']['session']['version'];
...
<script type="text/javascript">
		jQuery(document).ready(function(){
			...
			?>
					...
					<input type="hidden" name="MerchantID" value="<?php echo 'ANTERIOR001'; ?>">
