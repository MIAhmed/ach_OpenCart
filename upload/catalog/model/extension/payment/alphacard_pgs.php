...
class ModelExtensionPaymentAlphacardPGS extends Model {
    ...
    public function getGateway() {
        if ($this->config->get('payment_alphacard_pgs_gateway') != 'other') {
            return 'https://' . $this->config->get('payment_alphacard_pgs_gateway') . '-gateway.alphacard.com';
        } else {
            $url_parts = parse_url($this->config->get('payment_alphacard_pgs_gateway_other'));
    ...
    public function getMethod($address, $total) {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_alphacard_pgs_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

        if ($this->config->get('payment_alphacard_pgs_total') > 0 && $this->config->get('payment_alphacard_pgs_total') > $total) {
            $status = false;
        } elseif (!$this->config->get('payment_alphacard_pgs_geo_zone_id')) {
            $status = true;
        ...
        if ($status) {
            $title_config = $this->config->get('payment_alphacard_pgs_display_name');
            ...
            $method_data = array(
                ...
                'terms'                            => '',
                'sort_order'                    => $this->config->get('payment_alphacard_pgs_sort_order')
            );
        }
    ...
    public function getCards($customer_id) {
        ...
        foreach ($this->getTokens($customer_id) as $token_data) {
            ...
            if (
                ...
                $this->db->query("DELETE FROM `" . DB_PREFIX . "alphacard_pgs_token` WHERE alphacard_pgs_token_id='" . (int)$token_data['alphacard_pgs_token_id'] . "'");
            } else {
                $result[] = array(
    ...
    public function getTokens($customer_id) {
        $tokens_result = $this->db->query("SELECT * FROM `" . DB_PREFIX . "alphacard_pgs_token` WHERE customer_id='" . (int)$customer_id . "'");
    ...
    public function saveToken($order_id, $token, $session_id, $sourceOfFunds) {
        ...
        if (!empty($response['result']) && $response['result'] == 'SUCCESS' && $response['status'] == 'VALID') {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "alphacard_pgs_token` SET `customer_id`='" . (int)$customer_id . "', `token`='" . $this->db->escape($token) . "', `date_added`=NOW()");
        } else {
            ...
            if (!empty($response_second['result']) && $response_second['result'] == 'SUCCESS' && $response_second['status'] == 'VALID') {
                $this->db->query("INSERT INTO `" . DB_PREFIX . "alphacard_pgs_token` SET `customer_id`='" . (int)$customer_id . "', `token`='" . $this->db->escape($response_second['token']) . "', `date_added`=NOW()");
            }
        }
    }
    ...
    public function initCheckoutSession($order_id, $currency) {
        ...
        $data['order']['notificationUrl'] = $this->url->link('extension/payment/alphacard_pgs/callback', '', true);

        $response = $this->api('session', $data, 'POST');

        if (!empty($response['result']) && $response['result'] == 'SUCCESS') {
            $this->session->data['alphacard_pgs'] = $response;
    ...
    public function clearCheckoutSession() {
        unset($this->session->data['alphacard_pgs']);
    }
    ...
    public function api($api_method, $data = array(), $method = 'GET') {
        ...
        $url = $gateway . $this->config->get('payment_alphacard_pgs_merchant') . '/' . $api_method;
        
        $userid = 'merchant.' . $this->config->get('payment_alphacard_pgs_merchant');
        $password = $this->config->get('payment_alphacard_pgs_integration_password');
