...
class ModelExtensionPaymentAlphacardPGS extends Model {
    ...
    public function createTables() {
        $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "alphacard_pgs_transaction` (
         `alphacard_pgs_transaction_id` int(11) unsigned NOT NULL AUTO_INCREMENT,
         ...
         PRIMARY KEY (`alphacard_pgs_transaction_id`),
         KEY `transaction_id_order_id` (`transaction_id`,`order_id`)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8");

        $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "alphacard_pgs_token` (
         `alphacard_pgs_token_id` int(11) unsigned NOT NULL AUTO_INCREMENT,
         `customer_id` int(11) NOT NULL,
         `token` char(40) NOT NULL,
         `date_added` datetime NOT NULL,
         PRIMARY KEY (`alphacard_pgs_token_id`),
         KEY `customer_id` (`customer_id`)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8");
    }

    public function dropTables() {
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "alphacard_pgs_transaction`");
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "alphacard_pgs_token`");
    }

    public function hookEvents() {
        $events = array(
            'catalog/controller/checkout/checkout/before' => 'extension/payment/alphacard_pgs/init',
            'admin/model/customer/customer/deleteCustomer/before' => 'extension/payment/alphacard_pgs/delete_tokens'
        );

        $this->load->model('setting/event');

        foreach ($events as $trigger => $action) {
            $this->model_setting_event->addEvent('payment_alphacard_pgs', $trigger, $action, 1, 0);
        }
    }
    ...
    public function unhookEvents() {
        ...
        $this->model_setting_event->deleteEventByCode('payment_alphacard_pgs');
    }

    public function getTotalTransactions($data) {
        $sql = "SELECT COUNT(*) as total FROM `" . DB_PREFIX . "alphacard_pgs_transaction`";
    ...
    public function deleteTokens($customer_id) {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "alphacard_pgs_token` WHERE customer_id='" . (int)$customer_id . "'");
    }

    public function getTransactions($data) {
        $sql = "SELECT * FROM `" . DB_PREFIX . "alphacard_pgs_transaction`";
    ...
    public function getTransaction($alphacard_pgs_transaction_id) {
        return $this->db->query("SELECT * FROM `" . DB_PREFIX . "alphacard_pgs_transaction` WHERE alphacard_pgs_transaction_id='" . (int)$alphacard_pgs_transaction_id . "'")->row;
    }
